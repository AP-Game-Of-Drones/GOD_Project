...
pub struct ChatClient
{
    ...
    pub current_server: Option<NodeId>,
...
impl ChatClient {
    ...
    fn set_server(&mut self, id: NodeId) {
    ...
    fn process_respsonse(&mut self, response: Message, session_id: u64, src_id: NodeId) -> Result<ProcessChatResults,ProcessChatResults> {
        if self.holder_sent.get(&(session_id,self.id)).is_some() {
            match response {
                ...
                Message::ChatMessages(cm) =>{
                    self.received.insert((session_id,src_id),Message::ChatMessages(cm.clone()));
            ...
            let serv = self.client_topology.get_all_servers();
                if !serv.is_empty() {
                    for s in serv {
                        ...
                        self.send_get_server_type(s).ok();
                    }
                }
...
fn fragment_packetization(fragments: &mut Vec<Fragment>, hops: Option<Vec<u8>>, session_id: u64)->Vec<Packet> {
    ...
    if hops.is_some(){
        for f in fragments {
            ...
            println!(" Pacjet {:?}, session : {} ",packet.clone(),session_id);
            vec.push(packet);
        }
    }
    vec
...
mod tests {
    ...
    fn test_send_chat_client(){
        let (c1,c2) = unbounded::<NodeCommand>();
        let (c3,c4) = unbounded::<NodeEvent>();
        let (c5,c6) = unbounded::<Packet>();
        
        let mut hm = HashMap::new();
        hm.insert(1,c5);;
        let mut dummy = ChatClient::new(0,c3,c2,c6,hm);
        dummy.client_topology.update_topology((0,NodeType::Client),vec![(1,NodeType::Server)]);
        let fragments=Message::ChatMessages(ChatMessages::CHATSTRING(1, 0 , "Hello".to_string()));
