...
pub fn spawn_drone(
    ...
    if let MainState::Sim = *state {
        for SpawnDroneEvent { id } in reader.read() {
...
pub fn update_nodes(
    ...
    if let MainState::Sim = *state {
        for _ in reader.read() {
            ...
            let drone_num = node_query
                .iter()
                .filter(|(node, _)| matches!(node.node_type, components::NodeType::Drone))
                .count();
            if drone_num > 1 {
                let mut i = 0;
            ...
            let client_num = node_query
                .iter()
                .filter(|(node, _)| matches!(node.node_type, components::NodeType::Client))
                .count();
            ...
            let x = (WINDOW_WIDTH / 8.0) * 3.0;
            let server_num = node_query
                .iter()
                .filter(|(node, _)| matches!(node.node_type, components::NodeType::Server))
                .count();
                    ...
                    for (c_node, c_transform) in node_query.iter() {
                        if c_node.id == *connected_node {
                            create_line(
                                ...
                                c_transform.translation.x,
                                c_transform.translation.y,
                            );
...
pub fn create_line(
    ...
    let line_positions = vec![Vec3::new(x1, y1, -1.0), Vec3::new(x2, y2, -1.0)];

    let mesh = Mesh::new(PrimitiveTopology::LineList, RenderAssetUsages::default())
        .with_inserted_attribute(Mesh::ATTRIBUTE_POSITION, line_positions);
...
pub fn crash_target(
    ...
    if let MainState::Sim = *state {
        ...
        let Ok((entity_to_remove, node_to_remove)) = node_query.get_mut(clicked_entity) else {
            return;
        };
        ...
        crash(
            ...
            node_to_remove.id,
            node_to_remove.connected_node_ids.clone(),
        );
        ...
        for (entity, mut node) in node_query.iter_mut() {
            if entity != entity_to_remove {
                node.connected_node_ids.retain(|&id| id != id_to_remove);
            }
        }
...
pub fn connect_nodes(
    ...
    if let MainState::Sim = *state {
        ...
        if let (Some(mut first_node), Some(mut second_node)) = (first_node_opt, second_node_opt) {
            if !first_node.connected_node_ids.contains(&second_node.id)
                && !second_node.connected_node_ids.contains(&first_node.id)
            {
                if first_node.node_type == components::NodeType::Drone
                    || second_node.node_type == components::NodeType::Drone
                {
...
pub fn button_action(
    mut drone_add_query: Query<
        &mut Visibility,
        (With<DroneAdd>, Without<ConfirmShown>, Without<TextBox>),
    >,
    mut textbox_query: Query<
        &mut Visibility,
        (With<TextBox>, Without<ConfirmShown>, Without<DroneAdd>),
    >,
    mut confirm_query: Query<
        &mut Visibility,
        (With<ConfirmShown>, Without<DroneAdd>, Without<TextBox>),
    >,
    ...
    interaction_query: Query<
        (&Interaction, Option<&ButtonLabel>),
                        ...
                        if *active_mode == ActiveMode::Add {
                            if let Ok(id) = value.0.parse::<u8>() {
                                if !node_query.iter().any(|(node, _)| node.id == id) {
                                    ...
                                    spawn_writer.write(SpawnDroneEvent { id });
                                } else {
                                    warn = true;
                                    ...
                                    if node.node_type == components::NodeType::Drone {
                                        if let Ok(pdr_val) = value.0.parse::<f32>() {
                                            if pdr_val >= 0.0 && pdr_val <= 1.0 {
                                                ...
                                                warn = true;
                                                *warn_text = Text::from(format!(
                                                    "Invalid PDR value: '{}' (must be between 0.0 and 1.0)",
                                                    value.0
                                                ));
        ...
        for create_event in reader.read() {
            ...
            match create_event {
                PacketCreateEvent::NodeEvent(NodeEvent::PacketSent(packet)) => {
                    ...
                    if let Some((_, _, start_transform)) =
                        node_query.iter().find(|(_, node, _)| node.id == start_id)
                    {
                        if let Some((_, _, end_transform)) =
                            node_query.iter().find(|(_, node, _)| node.id == end_id)
                        {
                            commands.spawn((
        ...
        for (entity, mut transform, mut sprite, mut motion, mut queue) in &mut packet_query {
            ...
            if motion.progress >= 1.0 {
                if let Some((_, next_end_id)) = queue.0.front() {
                    match next_end_id {
                        Some(id) => {
                            if let Some((_, end_transform)) =
                                node_query.iter().find(|(node, _)| node.id == *id)
                            {
                        ...
                        None => {
                            let alpha =
                                (sprite.color.alpha() - time.delta_secs() * 0.5).clamp(0.0, 1.0);
